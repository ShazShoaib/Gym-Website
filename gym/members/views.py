...
def your_classes(request):
    ...
    try:
        ...
        services = service.objects.filter(id__in=[booking.service_field_id for booking in bookings])
        # Join the services with the bookings based on both their ids
        bookings_with_session_status = bookings.annotate(session_status=F('service_field__session_status'))
        booking_dict = {booking.service_field_id: booking for booking in bookings_with_session_status}
        return render(request, "members/your_classes.html", {
            "bookings": bookings_with_session_status,
...
def trainer_services(request):
    ...
    for service_obj in services:
        service_obj.bookings_count = booking.objects.filter(service_field=service_obj).count()
    return render(request, "members/trainer_services.html", {
        "services": services
    })
...
def toggle_session_status(request, service_id):
    service_obj = service.objects.get(id=service_id)
    service_obj.session_status = not service_obj.session_status
    service_obj.save()
    return redirect('trainer_services')
